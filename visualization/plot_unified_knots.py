"""Generate outputs/plots/unified_plot_knots.png."""
...
OUT = ROOT / "outputs" / "plots" / "unified_plot_knots.png"
...
def load_db(path):
    with sqlite3.connect(path) as conn:
        minima = pd.read_sql_query(
            """
            SELECT knots.name, MIN(bidegrees.smallest_nonzero) AS min_gap
            FROM knots
            JOIN bidegrees ON knots.id = bidegrees.knot_id
        ...
        all_gaps = pd.read_sql_query(
            """
            SELECT knots.name, bidegrees.smallest_nonzero AS gap
            FROM knots
            JOIN bidegrees ON knots.id = bidegrees.knot_id
            WHERE bidegrees.smallest_nonzero IS NOT NULL
              AND bidegrees.smallest_nonzero > 0
...
def main():
    min_13, all_13 = load_db(ROOT / "databases" / "knot_research.db")
    min_14, all_14 = load_db(ROOT / "databases" / "knot_research_14.db")
    minima = pd.concat([min_13, min_14], ignore_index=True)
    all_gaps = pd.concat([all_13, all_14], ignore_index=True)
    minima = minima[minima["crossings"] <= 14]
    all_gaps = all_gaps[all_gaps["crossings"] <= 14]
    ...
    ax.set_title("Knot Khovanov Hodge Laplacian Spectral Gaps")
